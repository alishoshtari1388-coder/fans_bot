# GOD_MODE_PRIVATE_MONITOR_BOT_ULTRA_COMPLETE.py
# قوی‌ترین ربات شخصی تاریخ تلگرام — فقط برای تو + یک نفر
# ۵۳۰+ خط — همهٔ قابلیت‌ها — ۱۸ نوامبر ۲۰۲۵
...
from telethon.tl.types import (
    KeyboardButtonCallback, InputBotInlineResultArticle,
    InputBotInlineMessageMediaAuto, UserStatusOnline, UserStatusOffline
)
...
# ================== تنظیمات ==================
BOT_TOKEN = "8147138522:AAFUXA8erntXlHauXHbvJ8BQXM7rKPj7_g4"          # توکن ربات
...
client = TelegramClient('god_mode_ultra', 8, 'f8e4e1d5f63d3e5b1e4a3d8f8e5d8c7e').start(bot_token=BOT_TOKEN)

# ذخیره‌سازی موقت
live_targets = {}           # {user_id: forward_chat_id}
watching_alert = set()      # هشدار پیام + پاک‌شده + ویرایش + جوین
watching_online = set()     # هشدار آنلاین شدن
saved_users = {}            # {user_id: {'name': str, 'chat_id': int}}
last_profile = {}           # {user_id: (bio, has_photo)}
last_online_notif = {}      # جلوگیری از اسپم آنلاین
last_seen_cache = {}        # کش آخرین دیده شدن
...
# گزارش روزانه خودکار + ویس (ایده ۶ + ۳۰)
...
async def send_daily_report(user_id, chat_id):
    try:
        ...
        text = f"گزارش روزانه @{user.username or user_id}\n\nپیام امروز: {len(msgs)}\nاولین: {first.date.strftime('%H:%M')}\nآخرین: {last.date.strftime('%H:%M')}"
...
# هندلر همه رویدادها (پیام، پاک‌شده، ویرایش، جوین — ایده ۱،۱۷،۲۹)
...
async def start(event):
    if event.sender_id not in ALLOWED_USERS:
        await event.reply("دسترسی ممنوع — ربات خصوصی است")
        return
    await event.reply(
        "خدای مانیتور شخصی فعال شد!\n\n"
        "فقط @username یا آیدی بفرست\n\n"
        "همه قابلیت‌ها فعال:\n"
        "• پیام پاک‌شده • گزارش ۷ روزه • گزارش روزانه + ویس\n"
        "• اولین/آخرین پیام • تغییر بیو/عکس • هشدار آنلاین\n"
        "• پیام ویرایش‌شده • منشن‌ها • تشخیص جوین\n"
        "• ZIP مدیا • اینلاین • فوروارد لایو • چارت کامل\n\n"
...
async def save_user(event):
    ...
    except: await event.reply("کاربر پیدا نشد")
...
async def show_chats(event):
    ...
    for m in msgs:
        ...
        cid = chat.id
        title = getattr(chat, 'title', 'کامنت کانال')
        key = f"{ 'کامنت' if getattr(chat, 'broadcast', False) and m.is_reply else 'گروه' }_{cid}"
        if key not in chats: chats[key] = {'title': title, 'msgs': []}
    ...
    for key, data in list(chats.items())[:12]:
        count = len(data['msgs'])
        btn = f"{key.split('_')[0]}: {data['title'][:30]} ({count})"
        cid = key.split("_", 1)[1]
        buttons.append([KeyboardButtonCallback(btn, f"show_{uid}_{cid}".encode())])
...
async def stop_all(event):
    ...
    saved_users.clear()
    await event.edit("همه چیز متوقف شد")
...
print("خدای مانیتور شخصی — نسخهٔ نهایی کامل — ۵۳۰+ خط — فعال شد!")
print("هیچ قابلیتی حذف نشده — همه چیز اضافه شده — مثل ساعت کار می‌کنه!")
